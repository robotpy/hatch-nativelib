...
def relink_pkgconf_package(pkg: Package, absolute_ok: bool, pcache: PackageCache):
    ...
    if absolute_ok:
        ...
        remap_abs: T.Dict[str, str] = {}
        for req in reqs:
            for reqlib in req.shared_libs:
                if reqlib.file_path.name in remap_abs:
                    assert remap_abs[reqlib.file_path.name] == str(reqlib.file_path)
                else:
                    remap_abs[reqlib.file_path.name] = str(reqlib.file_path)

        for lib in pkg.shared_libs:
            for install_name, new_install_name in _iter_install_names(
                lib.file_path, remap_abs
            ):
        ...
        remap_rel: T.Dict[str, Library] = {}
        for req in reqs:
            for reqlib in req.shared_libs:
                if reqlib.file_path.name in remap_rel:
                    assert (
                        remap_rel[reqlib.file_path.name].file_path == reqlib.file_path
                    )
                else:
                    remap_rel[reqlib.file_path.name] = reqlib

        for lib in pkg.shared_libs:
            assert lib.dist_path is not None
            for install_name, resolved in _iter_install_names(lib.file_path, remap_rel):
